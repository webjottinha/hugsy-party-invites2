...
import { RsvpForm, type FormField } from "@/components/rsvp-form";
...
export function Invitation({ eventData, formUrl, formFields }: InvitationProps) {
    ...
    <main className="overflow-hidden bg-background">
      ...
      <section className="relative bg-card px-4 py-16 sm:px-7 sm:py-20">
        <div className="mx-auto max-w-5xl">
          ...
          {eventData.note && (
            <div className="party-card mx-auto mt-6 flex max-w-3xl items-start gap-4 p-5 sm:p-6">
              <span className="icon-disc icon-pink flex size-12 shrink-0 items-center justify-center"><Heart className="size-6 fill-current" /></span>
              <p className="text-base font-bold leading-relaxed text-foreground/85 sm:text-lg">{eventData.note}</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section ref={rsvpRef} id="confirmar" className="rsvp-section relative scroll-mt-0 px-4 py-16 sm:px-7 sm:py-24">
        ...
        <div className="relative z-10 mx-auto max-w-3xl rounded-[2rem] border-4 border-card bg-card/95 p-6 shadow-party backdrop-blur-sm sm:p-10 lg:p-14">
          <RsvpForm formUrl={formUrl} fields={formFields} />
        </div>
      </section>
